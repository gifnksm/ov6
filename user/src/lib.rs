...
macro_rules! message {
    ($($msg:tt)*) => {
        {
            let prog = ::xv6_user_lib::env::arg0();;
            ::xv6_user_lib::eprintln!("{prog}: {msg}", msg = ::core::format_args!($($msg)*));
        }
    }
}
...
macro_rules! usage_and_exit {
    ($($args:tt)*) => {
        {
            let prog = ::xv6_user_lib::env::arg0();;
            ::xv6_user_lib::eprintln!("Usage: {prog} {args}", args = ::core::format_args!($($args)*));
            ::xv6_user_lib::process::exit(1);
        }
    };
}
